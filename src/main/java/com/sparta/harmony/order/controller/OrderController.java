...
public class OrderController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "주문 요청이 성공적으로 접수된 경우", content = @Content(schema = @Schema(implementation = OrderResponseDto.class), mediaType = "application/json")),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "주문 조회가 성공적으로 이뤄진 경우", content = @Content(schema = @Schema(implementation = OrderResponsePageDto.class), mediaType = "application/json")),
    ...
    @PreAuthorize("hasAuthority('ROLE_USER') or hasAuthority('ROLE_OWNER')  or hasAuthority('ROLE_MANAGER') or hasAuthority('ROLE_MASTER')")
    @GetMapping("/orders")
    public ResponseEntity<OrderResponsePageDto> getOrders(
        ...
        OrderResponsePageDto orderResponsePageDto = orderService.getOrders(userDetails.getUser(), page - 1, size, sortBy, isAsc);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "주문 조회가 성공적으로 이뤄진 경우", content = @Content(schema = @Schema(implementation = OrderDetailResponseDto.class), mediaType = "application/json")),
            @ApiResponse(responseCode = "400", description = "주문 조회에 실패했을 경우", content = @Content(schema = @Schema(implementation = RestApiException.class), mediaType = "application/json")),
    })
    @Parameter(name = "orderId", description = "주문 ID", example = "92e09e78-d0fd-4fdd-b335-4971d501bf6c")
    @PreAuthorize("hasAuthority('ROLE_USER') or hasAuthority('ROLE_OWNER')  or hasAuthority('ROLE_MANAGER') or hasAuthority('ROLE_MASTER')")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "수정이 성공적으로 이뤄진 경우", content = @Content(schema = @Schema(implementation = OrderResponseDto.class), mediaType = "application/json")),
